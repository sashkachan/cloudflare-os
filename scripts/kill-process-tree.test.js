...
describe("killProcessTree", () => {
  it("rejects a non-numeric pid rather than signalling a process group", async () => {
    await assert.rejects(killProcessTree("not-a-pid"), /pid must be a positive integer/);
  });
  ...
  it("rejects pids that would address process groups or only look numeric", async () => {
    ...
    // parseInt-style coercion would also accept "12abc" and signal an unrelated pid 12.
    for (const pid of [0, -123, "12abc", "123", 12.5, undefined]) {
      await assert.rejects(killProcessTree(pid), /pid must be a positive integer/);
    }
  });
